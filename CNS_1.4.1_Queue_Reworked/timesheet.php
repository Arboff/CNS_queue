...
if (isset($_GET['action']) && $_GET['action']==='export_csv') {
    ...
    fputcsv($output, ['User', 'Start', 'End', 'Duration (HH:MM:SS)']);

    foreach ($rawEntries as $entry) {
        $startFormatted = date("d-m-Y H:i:s", $entry['start']);
        $endFormatted = date("d-m-Y H:i:s", $entry['end']);
        $durationFormatted = formatDuration($entry['duration']);
        fputcsv($output, [$entry['user'], $startFormatted, $endFormatted, $durationFormatted]);
    }
...
if (isset($_GET['action']) && $_GET['action'] === 'export_overrides_csv') {
    $overrideData = file_exists(__DIR__."/overrides.json") ? json_decode(file_get_contents(__DIR__."/overrides.json"), true) : [];

    $filterOverrider = $_GET['overrider'] ?? '';
    $filterSkipped = $_GET['skipped'] ?? '';
    $startDate = $_GET['start_date'] ?? '';
    $endDate = $_GET['end_date'] ?? '';

    if ($filterOverrider) $overrideData = array_filter($overrideData, fn($o) => $o['overrider'] === $filterOverrider);
    if ($filterSkipped) $overrideData = array_filter($overrideData, fn($o) => $o['skipped_user'] === $filterSkipped);
    if ($startDate) $overrideData = array_filter($overrideData, fn($o) => $o['timestamp'] >= strtotime($startDate));
    if ($endDate) $overrideData = array_filter($overrideData, fn($o) => $o['timestamp'] <= strtotime($endDate . ' 23:59:59'));

    header('Content-Type: text/csv; charset=utf-8');
    header('Content-Disposition: attachment; filename="overrides_history.csv"');

    $output = fopen('php://output', 'w');
    fputcsv($output, ['Timestamp', 'Overrider', 'Skipped User', 'Reason']);

    foreach ($overrideData as $o) {
        $timeFormatted = date("d-m-Y H:i:s", $o['timestamp']);
        fputcsv($output, [$timeFormatted, $o['overrider'], $o['skipped_user'], $o['reason']]);
    }
...
if (isset($_GET['action']) && $_GET['action']==='export_unplanned_csv') {
    $filtered = $unplannedBreaks;
    $filterUser = $_GET['user'] ?? '';
    $startDate = $_GET['start_date'] ?? '';
    $endDate = $_GET['end_date'] ?? '';
    ...
    header('Content-Type: text/csv; charset=utf-8');
    header('Content-Disposition: attachment; filename="unplanned_breaks.csv"');
    ...
    foreach ($filtered as $b) {
        ...
        $endFormatted = date("d-m-Y H:i:s", $b['end']);
        $durationFormatted = formatDuration($b['duration']);
        fputcsv($output, [$b['username'], $startFormatted, $endFormatted, $durationFormatted]);
    }
...
<?php
$unplannedUsers = array_unique(array_column($unplannedBreaks, 'username'));
sort($unplannedUsers);
?>
...
<div class="section">
    ...
    <option value="">-- All Users --</option>
    <?php foreach($unplannedUsers as $u): ?>
    ...
    <!-- Clear All Unplanned Breaks Button -->
    <form method="post" onsubmit="return confirm('Clear all unplanned breaks?');" style="margin-top:10px;">
        <button type="submit" name="clear_unplanned" class="reset-btn">Clear All Unplanned Breaks</button>
    </form>
</div>
...
foreach (file_exists(__DIR__."/overrides.json") ? json_decode(file_get_contents(__DIR__."/overrides.json"), true) : [] as $o) {
    $overrideUsers[] = $o['overrider'];
    $skippedUsers[] = $o['skipped_user'];
}

$overrideUsers = array_unique($overrideUsers);
$skippedUsers = array_unique($skippedUsers);
...
<div class="section">
    ...
    <?php foreach($overrideUsers as $u): ?>
        <option value="<?=htmlspecialchars($u)?>"><?=htmlspecialchars($u)?></option>
    <?php endforeach; ?>
</select>
    ...
    <?php endforeach; ?>
        ...
        <button type="button" id="exportOverrides">Export Overrides</button>
...
<script>
    ...
    document.getElementById('exportOverrides').addEventListener('click', () => {
    ...
    window.location.href = `timesheet.php?action=export_overrides_csv&overrider=${encodeURIComponent(overrider)}&skipped=${encodeURIComponent(skipped)}&start_date=${start_date}&end_date=${end_date}`;
});
...
document.getElementById('exportBtn').addEventListener('click',()=>{
    ...
    const start_date = document.getElementById('start_date').value;
    const end_date = document.getElementById('end_date').value;
    window.location.href = `timesheet.php?action=export_csv&user=${encodeURIComponent(user)}&start_date=${start_date}&end_date=${end_date}`;
});
...
document.getElementById('exportUnplanned').addEventListener('click',()=>{
    ...
    const start_date = document.getElementById('unplanned_start').value;
    const end_date = document.getElementById('unplanned_end').value;
    window.location.href = `timesheet.php?action=export_unplanned_csv&user=${encodeURIComponent(user)}&start_date=${start_date}&end_date=${end_date}`;
});
...
<div class="section">
    <h2>UNRELEASED / WIP FEATURES</h2>
    <div style="text-align:center; margin-top:10px;">
        <div class="wip-buttons">
            <button data-tooltip="Information above, but plotted into graphs instead of table (WIP)" onclick="window.location.href='graphs.php'">Graph View (WIP)</button>
            <button data-tooltip="Send broadcast messages to all users" onclick="window.location.href='broadcast.php'">Broadcast Message</button>
            <button data-tooltip="Page to arrange breaks with collegues (WIP)" onclick="window.location.href='breaks.php'">Breaks Manager (WIP)</button>
            <button data-tooltip="Feedback submission and managment" onclick="window.location.href='feedback.php'">Feedback Submission</button>
            <button data-tooltip="Toggle maintenance mode // Lock platform" onclick="window.location.href='power.php'">Toggle Maintenance Mode</button>
            <button data-tooltip="Clear the queue manually" onclick="window.location.href='cron_job.php'">Clear Queue</button>
            <button data-tooltip="Self service portal for changing password" onclick="window.location.href='password_reset.php'">Password Reset Portal</button>
            <button data-tooltip="View raw user data (Passwords and Admin status)" onclick="window.location.href='user_list.php'">Raw Users</button>
            <button data-tooltip="View raw user data (Passwords and Admin status)" onclick="window.location.href='login_screen.php'">Login Screen Splash</button>
        </div>
        <div class="wip-description">
            Features which are either WIP or fully working but not intended for production use
        </div>
    </div>
</div>
...
.wip-buttons button {
    position: relative;
    background: #4a90e2; /* Match other buttons */
    color: #fff;
    border: none;
    padding: 10px 18px;
    border-radius: 8px;
...
<script>
// Tooltip with 2s delay
let tooltipTimeout;
const tooltipDiv = document.createElement('div');
tooltipDiv.className = 'tooltip';
document.body.appendChild(tooltipDiv);

document.querySelectorAll('.wip-buttons button').forEach(btn => {
    btn.addEventListener('mouseenter', e => {
        const text = btn.getAttribute('data-tooltip');
        tooltipTimeout = setTimeout(() => {
            tooltipDiv.textContent = text;
            const rect = btn.getBoundingClientRect();
            tooltipDiv.style.top = rect.top - tooltipDiv.offsetHeight - 6 + window.scrollY + 'px';
            tooltipDiv.style.left = rect.left + (rect.width - tooltipDiv.offsetWidth)/2 + window.scrollX + 'px';
